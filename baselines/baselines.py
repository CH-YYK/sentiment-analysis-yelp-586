...
#
train_ = pd.read_csv("../data/business_reviews2017_train.tsv", sep='\t')
test_ = pd.read_csv("../data/business_reviews2017_test.tsv", sep='\t')

train_x, valid_x = train_['text'], test_['text']
train_y, valid_y = train_['class'], test_['class']

trainDF = pd.concat([train_, test_], axis=0)
...
accuracy = train_model(naive_bayes.GaussianNB(), xtrain_count, train_y, xvalid_count, valid_y)
