...
class data_tool(object):
    def __init__(self, data_path, truncated_length, train_set_ratio=0.8, dev_set_size=600):
        ...
        if os.path.isfile('dataset_y.npy'):
            ...
            # list_y = [-1]+[star for star in self.data['class'].apply(lambda x: int(x >= 3))]
            self.data_y = self.encoder_y.fit_transform(self.data['class'])
    ...
    def clean_data(self, data_path):
        ...
        """
        print('Load dataset...')
        data = pd.read_table(data_path, sep='\t', usecols=['text', 'class'])
        return data.dropna()
