...
class CharCNN(object):
    def __init__(self, sequence_length, char_vector, conv_config, fc_config, num_classes):
        l2_loss = 0
        # define Placeholders
        ...
        with tf.name_scope("loss_and_accuracy"):
            losses = tf.nn.softmax_cross_entropy_with_logits_v2(logits=self.scores, labels=self.label_y)
            self.loss = tf.reduce_mean(losses) + l2_loss * 0.001
