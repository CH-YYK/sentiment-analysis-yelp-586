...
class Training(data_tool, CharCNN):
    def __init__(self, data_path, truncated_length, conv_config, fc_config, batch_size, epoch_size,
                 outdir='./', check_dir=None):
        ...
        with tf.Graph().as_default():
            ...
            with sess.as_default():
                ...
                checkpoint_dir = os.path.join(outdir, 'CharCNN_runs', temp_dir, 'checkpoints')
                ...
                train_summary_dir = os.path.join(outdir, "CharCNN_runs", temp_dir, "summaries", "train")
                train_summary_writer = tf.summary.FileWriter(train_summary_dir, sess.graph)

                # Test Summaries
                test_summary_op = tf.summary.merge([loss_summary, acc_summary])
                test_summary_dir = os.path.join(outdir, 'CharCNN_runs', temp_dir, 'summaries', 'test')
                ...
                def test_():
                    feed_dict = {self.input_x: self.dev_x[:200],
                                 self.label_y: self.dev_y[:200],
                                 self.keep_prob: 1.0}
